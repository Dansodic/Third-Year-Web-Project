	...
	<!-- Header -->
		<div id="header">
			<div class="container">
				...
				<!-- Logo -->
					<div id="logo">
						<h1><a style="color:rgb(255, 255, 255);" href="#">The Vehicles</a></h1>
              ...
              <li class="active"><a href="vehicles.html.php">Vehicles</a></li>
					...
					<!-- Content -->
						<div id="content" class="12u skel-cell-important">
							<section>
								...
								<p>Welcome to the Star Wars Vehicles page. Below you will find various options to choose from. Each will give you information about each Vehicle related to the criteria specified by your choice.</p>
					
					<p>
          <form action = "vehicles.html.php" method = "post" name = "reportForm">
					<input type = "hidden" name = "choice">
					</form>

<h3>(Click a button to see specific information about each Vehicle.)</h3>
...
<input type = 'button' class = "button" id = "flight" value = 'Max Atmosphering Speed' 
...
function modeldata()
{
  ...
  $sql = "SELECT name, model,manufacturer FROM vehicles ORDER BY name";
...
{
  $connect = mysqli_connect("localhost", "root", "", "SWAPI"); //Connect PHP to MySQL Database
  $sql = "SELECT name,cost_in_credits,vehicle_class FROM vehicles ORDER BY name";
  $result = mysqli_query($connect, $sql);
      
  echo "<table id = 'customers'>
      <tr><th>Name</th><th>Vehicles Class</th><th>Cost in Credits</th></tr>";
  while ($row = mysqli_fetch_array($result))
  {   
      echo "<td>" . $row['name'] . "</td>
...
{
  $connect = mysqli_connect("localhost", "root", "", "SWAPI"); //Connect PHP to MySQL Database
  $sql = "SELECT name,cargo_capacity,crew,passengers,consumables,length FROM vehicles ORDER BY name";
  $result = mysqli_query($connect, $sql);
...
{
  $connect = mysqli_connect("localhost", "root", "", "SWAPI"); //Connect PHP to MySQL Database
  $sql = "SELECT name,max_atmosphering_speed FROM vehicles ORDER BY name";
  $result = mysqli_query($connect, $sql);
      
  echo "<table id = 'customers'>
      <tr><th>Name</th><th>Max Atmosphering Speed</th></tr>";
  while ($row = mysqli_fetch_array($result))
  {      
      echo "<td>" . $row['name'] . "</td>
