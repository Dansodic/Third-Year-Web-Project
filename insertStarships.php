...
//Code used to get contents of JSON file and decode it to PHP Object. Then iterate through the object using a foreach loop in order to insert data to database.
          ...
          foreach($object as $row) //Extract the Object Values using Foreach Loop
          {
           ...
           $query .= "INSERT INTO starships(name, model, manufacturer, cost_in_credits, length, max_atmosphering_speed, crew, passengers, cargo_capacity, consumables, hyperdrive_rating, MGLT, starship_class) VALUES (
            '".$row["name"]."', 
           '".$row["model"]."', 
           '".$row["manufacturer"]."', 
           '".$row["cost_in_credits"]."', 
